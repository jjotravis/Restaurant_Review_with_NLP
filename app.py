...
# async def login_for_access_token(form_data: OAuth2PasswordRequestForm = Depends(), service: Session = Depends(get_user_service), Autho: Session = Depends(get_auth_service)):
#     user = service.get_user_by_username(form_data.username)
    
#     if not user or not Autho.verify_password(form_data.password, user.password):
#         raise HTTPException(
...
#     access_token = Autho.create_access_token(
...
# def get_all_admins(
#     current_user: UserBase = Depends(get_auth_service),  # Ensure only admins access this
...
# @app.get("/api/user/username/{username}", response_model=UserResponseLogin)
...
@app.post("/restaurant_review/token", response_model=dict)
async def login_for_access_token(
    form_data: OAuth2PasswordRequestForm = Depends(), 
    service: Session = Depends(get_user_service), 
...
@app.post("/restaurant_review/admin/add", response_model=AdminResponse)
def create_admin(admin: AdminCreate, service: AdminService = Depends(get_admin_service)):
    return service.create_admin(admin)
...
@app.get("/restaurant_review/reviews/user/{user_id}", response_model=List[ReviewResponse])
async def get_reviews_by_user(user_id: int, service: ReviewService = Depends(get_review_service)):
    return service.get_reviews_by_user_id(user_id)
